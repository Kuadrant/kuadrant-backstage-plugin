...
test.describe("Kuadrant Permissions Matrix", () => {
  ...
  test.describe("APIKey Permissions", () => {
    ...
    test("kuadrant.apikey.delete.own - consumer sees My API Keys card with tabs", async ({ page }) => {
      ...
      // find My API Keys card - consumer should always see this
      ...
      // verify tabs exist (pending, approved, rejected)
      const pendingTab = myApiKeysCard.getByRole("tab", { name: /pending/i });
      const approvedTab = myApiKeysCard.getByRole("tab", { name: /approved/i });

      // at least one tab should be visible (structure is correct)
      const hasTabs = await pendingTab.or(approvedTab).first().isVisible().catch(() => false);
      expect(hasTabs, "My API Keys card should have status tabs").toBe(true);
    });
  });
