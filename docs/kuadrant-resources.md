...
**Dev cluster:** MCP resources are exercised on the [`oinc`](../oinc/) cluster, which installs the real `mcp-gateway` addon — so the `mcp.kuadrant.io` CRDs and server statuses come from the operator rather than hand-vendored copies — and seeds a demo `MCPServerRegistration` via [`oinc/manifests/mcp-demo.yaml`](../oinc/manifests/mcp-demo.yaml). Both host and in-cluster RHDH service accounts use the shared [`rhdh-cluster-role.yaml`](../kuadrant-dev-setup/rbac/rhdh-cluster-role.yaml), which grants `get/list/watch` on MCP resources and Gateway API resources. Run `oinc/setup.sh` to create the cluster and deploy RHDH; see [oinc.md](oinc.md).
